        ...
        function updateHighlighting() {
            ...
            if (syllableFilter === 'grace_main_units') {
                ...
                if (analysisData.grace_main_units.length > 1) {
                    drawGraceAwareConnections(analysisData.grace_main_units, '#2e7d32', 2, 'grace-main-units');
                }
                ...
                Object.entries(analysisData.melismatic_syllables).forEach(([syllable, positions]) => {
                    ...
                    // Draw connection lines only for main notes within each melismatic syllable to avoid tangles
                    if (positions.length > 1) {
                        // Filter to main notes only for cleaner connections
                        const mainNotePositions = positions.filter(pos => {
                            const note = document.getElementById(`note-${pos}`);
                            return note && note.getAttribute('data-is-grace') !== 'true';
                        });
                        
                        if (mainNotePositions.length > 1) {
                            drawPatternConnections(mainNotePositions, '#6a1b9a', 2, `melisma-${syllable}`);
                        }
                    }
                });
                ...
                if (analysisData.tone_syllables && analysisData.tone_syllables[tone]) {
                    ...
                    if (analysisData.tone_syllables[tone].length > 1) {
                        drawGraceAwareConnections(analysisData.tone_syllables[tone], toneColors[tone], 2, `tone-${tone}`);
                    }
                ...
                Object.entries(analysisData.repeated_syllables).forEach(([syllable, data]) => {
                    if (data.count >= minCount) {
                        ...
                        if (data.positions.length > 1) {
                            drawGraceAwareConnections(data.positions, '#f44336', 2, `repetition-${syllable}`);
                        }
                    }
                });
