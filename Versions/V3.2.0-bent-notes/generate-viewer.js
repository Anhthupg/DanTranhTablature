...
function generateSinglePartSVG(partData, yOffset = 0, partNumber = null) {
    ...
    <rect x="${note.x}" y="${adjustedY - 5}"
        ...
        // Simple bent note indicator - vertical line pointing down to note
        if (note.isBent) {
            const lineStartY = adjustedY - 30;  // Start 30px above the note
            const lineEndY = adjustedY - 5;     // End 5px above the note (pointing to it)

            svg += `
    <!-- Bent note indicator -->
    <g class="bent-note-group" data-note-index="${index}">
        <!-- Vertical line pointing down to bent note -->
        <line x1="${note.x}" y1="${lineStartY}"
              x2="${note.x}" y2="${lineEndY}"
              stroke="#E74C3C" stroke-width="2"
              class="bend-symbol"/>

        <!-- Arrow head pointing down -->
        <path d="M ${note.x - 4},${lineEndY - 4} L ${note.x},${lineEndY} L ${note.x + 4},${lineEndY - 4}"
              stroke="#E74C3C" stroke-width="2" fill="none"/>

        <!-- Bend indicator text -->
        <text x="${note.x}" y="${lineStartY - 5}"
              text-anchor="middle" class="bend-amount"
              style="font-size: 10px; fill: #E74C3C; font-weight: bold;">
            BEND
        </text>
    </g>`;
        }
...
<svg id="tablatureSvg" xmlns="http://www.w3.org/2000/svg" width="${maxX}" height="${svgHeight}">
    ...
    <text x="${note.x + 4}" y="${adjustedY + 40}"
        ...
        // Simple bent note indicator for multi-part - vertical line pointing down
        if (note.isBent) {
            const lineStartY = adjustedY - 30;  // Start 30px above the note
            const lineEndY = adjustedY - 5;     // End 5px above the note
