...
function generateSinglePartSVG(partData, yOffset = 0, partNumber = null) {
    ...
    // Collect all bent indicators to draw them last
    ...
    <g class="bent-note-group" data-note-index="${index}">
        ...
        <text class="bent-arrow" x="${arrowEndX}" y="${arrowEndY}" text-anchor="middle"
              data-base-x="${arrowEndX}" data-base-y="${arrowEndY}" data-note-radius="${noteRadius}"
              style="font-size: 35px; fill: #FF0000; font-weight: bold; stroke: #FF0000; stroke-width: 1px;">↘</text>
        <!-- BEND text at starting position -->
        <text class="bent-text" x="${arrowStartX}" y="${arrowStartY + 5}" text-anchor="middle"
              data-base-x="${arrowStartX}" data-base-y="${arrowStartY + 5}"
...
<body>
    ...
    <script>
        ...
        function updateZoom() {
            ...
            // Update bent indicators - arrows and BEND text
            const bentArrows = svg.querySelectorAll('.bent-arrow');
            bentArrows.forEach(arrow => {
                // Get the associated note
                const bentGroup = arrow.closest('.bent-note-group');
                const noteIndex = bentGroup.getAttribute('data-note-index');
                const associatedNote = svg.querySelector(\`.note-circle[data-note-index="\${noteIndex}"]\`);

                if (associatedNote) {
                    // Get the note's current scaled position
                    const noteX = parseFloat(associatedNote.getAttribute('cx'));
                    const noteY = parseFloat(associatedNote.getAttribute('cy'));
                    const noteRadius = parseFloat(arrow.getAttribute('data-note-radius'));

                    // Position arrow tip at left edge of note
                    const arrowX = noteX - noteRadius;
                    arrow.setAttribute('x', arrowX);
                    arrow.setAttribute('y', noteY);
                }
            });

            const bentTexts = svg.querySelectorAll('.bent-text');
            bentTexts.forEach(text => {
                const baseX = parseFloat(text.getAttribute('data-base-x'));
                const baseY = parseFloat(text.getAttribute('data-base-y'));

                // Scale BEND text position
                const scaledX = 120 + (baseX - 120) * currentZoomX;
                const scaledY = baseY * currentZoomY;
        ...
        function toggleBentNotesHighlight() {
            ...
            allNotes.forEach(noteCircle => {
                ...
                // Look for bent-note-group instead of bend-symbol
                const bentGroup = svg.querySelector(\`.bent-note-group[data-note-index="\${noteIndex}"]\`);
                if (bentGroup) {
