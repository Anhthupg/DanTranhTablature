...
<head>
    ...
    <style>
        ...
        /* Theme-sensitive bent notes buttons */
        .bent-notes-button {
            margin-left: 20px;
            padding: 8px 16px;
            border-radius: 8px;
        ...
        .bent-notes-button.active {
            background: #E74C3C;
            color: var(--bg-color);
            box-shadow: 0 2px 12px rgba(231, 76, 60, 0.4);
        }

        .bent-notes-button.active:hover {
            background: #C0392B;
            border-color: #C0392B;
        }

        /* Black theme overrides */
        body.theme-black .bent-notes-button {
            border-color: #FF6B6B;
            color: #FF6B6B;
            box-shadow: 0 2px 4px rgba(255, 107, 107, 0.2);
        }

        body.theme-black .bent-notes-button:hover {
            background: #FF6B6B;
            color: #000000;
            box-shadow: 0 4px 8px rgba(255, 107, 107, 0.3);
        }

        body.theme-black .bent-notes-button.active {
            background: #FF6B6B;
            color: #000000;
            box-shadow: 0 2px 12px rgba(255, 107, 107, 0.5);
        }

        body.theme-black .bent-notes-button.active:hover {
            background: #FF5252;
            border-color: #FF5252;
        }
        ...
        .x-zoom-slider::-webkit-slider-thumb {
            appearance: none;
            width: 20px;
            height: 20px;
            border-radius: 50%;
        ...
        /* Blue fill effect for X-slider */
        .x-zoom-slider::-webkit-slider-runnable-track {
            background: linear-gradient(to right, #3498DB 0%, #3498DB var(--fill-percent, 50%), #E5E7EB var(--fill-percent, 50%), #E5E7EB 100%);
            height: 8px;
            border-radius: 5px;
        }
        ...
        .y-zoom-slider {
            width: 8px;
            height: 200px;
            background: linear-gradient(180deg, #85C1E9, #3498DB);
            border-radius: 5px;
            outline: none;
        ...
        .y-zoom-slider::-webkit-slider-thumb {
            appearance: none;
            width: 20px;
            height: 20px;
            border-radius: 50%;
...
<body>
    <div class="container">
        ...
        <div class="zoom-controls">
            <div class="y-zoom-group">
                <div class="zoom-label">Y</div>
                <input type="range" class="y-zoom-slider" min="10" max="200" value="100"
                       onchange="updateZoom('y', this.value); updateZoomDisplay('y', this.value)"
                       oninput="updateZoomDisplay('y', this.value)">
                <div class="zoom-value" id="y-zoom-value">100%</div>
            </div>
            <div class="x-zoom-group">
                <div class="zoom-label">X-Zoom</div>
                <input type="range" class="x-zoom-slider" min="10" max="200" value="100"
                       onchange="updateZoom('x', this.value); updateZoomDisplay('x', this.value)"
                       oninput="updateZoomDisplay('x', this.value)">
                <div class="zoom-value" id="x-zoom-value">100%</div>
            </div>
        </div>
        ...
        <div class="tuning-panel">
            ...
            <div class="panel-content expanded" id="optimalContent">
                <div class="tuning-info">
                    <span class="tuning-display" id="optimalTuning">${originalTuning.join('-')}</span>
                    <span class="bent-notes-count" id="optimalBentCount">${optimalSVG.bentNoteCount} bent notes</span>
                    <button id="optimalBentToggle" class="bent-notes-button" onclick="toggleBentNotesPanel('optimal')">
                        <span>Bent Notes</span>
                    </button>
                </div>
        ...
        <div class="tuning-panel">
            ...
            <div class="panel-content collapsed" id="traditionalContent">
                <div class="tuning-info">
                    ...
                    <button id="traditionalBentToggle" class="bent-notes-button" onclick="toggleBentNotesPanel('traditional')">
                        <span>Bent Notes</span>
                    </button>
                </div>
                <div class="tablature-container" id="traditionalContainer">
                    <svg width="${traditionalSVG.width}" height="${traditionalSVG.height}" id="comparisonSVG">
                        ${traditionalSVG.svg}
                    </svg>
                </div>
            </div>
        ...
        <div class="content-panel" id="analysisPanel">
            ...
            <div class="panel-content expanded" id="analysisPanelContent">
                ...
                <div class="content-section visible" id="lyricsContentSection">
                    ...
                    <div class="verse">Sample Vietnamese lyrics for ${songName}</div>
                    <h4>Romanized Vietnamese</h4>
                    <div class="verse">Sample romanized lyrics for ${songName}</div>
                    <h4>English Translation</h4>
                    <div class="verse">Sample English translation for ${songName}</div>
                </div>

                <!-- Musical Analysis Content -->
                <div class="content-section" id="analysisContentSection">
                    <h4>Musical Structure Analysis</h4>
                    <div class="verse">This song uses ${originalTuning.join('-')} tuning with ${relationshipData.metadata.totalNotes} total notes, including ${relationshipData.metadata.graceNotes} grace notes and ${relationshipData.melismaRelationships.length} melisma groups. The song structure demonstrates traditional Vietnamese pentatonic patterns.</div>
                </div>

                <!-- Performance Notes Content -->
                <div class="content-section" id="performanceContentSection">
                    <h4>Performance Techniques</h4>
                    <div class="verse">Key performance techniques for this piece include careful attention to grace note timing and the ${relationshipData.graceNoteRelationships.length} grace note relationships. The ${relationshipData.melismaRelationships.length} melisma groups require smooth transitions between sustained notes.</div>
                </div>

                <!-- Cultural Context Content -->
                <div class="content-section" id="culturalContentSection">
                    <h4>Cultural Context</h4>
                    <div class="verse">${songName} represents traditional Vietnamese musical heritage. This piece reflects the cultural values and artistic expression of Vietnamese folk music traditions.</div>
                </div>

                <!-- Historical Background Content -->
                <div class="content-section" id="historyContentSection">
                    <h4>Historical Background</h4>
                    <div class="verse">${songName} is part of the traditional Vietnamese folk song repertoire that has been preserved through oral tradition and written documentation. It represents the rich musical heritage of Vietnam.</div>
                </div>

                <!-- Regional Variations Content -->
                <div class="content-section" id="variationsContentSection">
                    <h4>Regional Variations</h4>
                    <div class="verse">Different regions of Vietnam may have variations in the performance style, ornamentation, and sometimes lyrical content of ${songName}. The current version represents one documented interpretation.</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function changeTuning(selectedTuning) {
            ...
            container.innerHTML = '<svg width="' + svgData.width + '" height="' + svgData.height + '" id="comparisonSVG">' + svgData.svg + '</svg>';
        ...
        function updateZoomDisplay(axis, value) {
            const displayElement = document.getElementById(axis + '-zoom-value');
            if (displayElement) {
                displayElement.textContent = Math.round(value) + '%';
            }

            // Update X-slider blue fill percentage
            if (axis === 'x') {
                const xSlider = document.querySelector('.x-zoom-slider');
                if (xSlider) {
                    const fillPercent = ((value - 10) / (200 - 10)) * 100; // Map 10-200 to 0-100%
                    xSlider.style.setProperty('--fill-percent', fillPercent + '%');
                }
            }
        }
        ...
        // Toggle bent notes visibility for each panel separately
        let optimalBentVisible = false;
        let comparisonBentVisible = false;

        function toggleBentNotesPanel(panelType) {
            console.log('toggleBentNotesPanel called with:', panelType);

            if (panelType === 'optimal') {
                optimalBentVisible = !optimalBentVisible;
                const button = document.getElementById('optimalBentToggle');
                const panelId = 'optimalSVG';
                console.log('Optimal panel - visible:', optimalBentVisible, 'button:', button, 'panel:', document.getElementById(panelId));

                if (optimalBentVisible) {
                    button.classList.add('active');
                    highlightBentNotesInPanel(panelId);
                } else {
                    button.classList.remove('active');
                    unhighlightBentNotesInPanel(panelId);
                }
            } else if (panelType === 'traditional') {
                comparisonBentVisible = !comparisonBentVisible;
                const button = document.getElementById('traditionalBentToggle');
                const panelId = 'comparisonSVG';
                console.log('Comparison panel - visible:', comparisonBentVisible, 'button:', button, 'panel:', document.getElementById(panelId));
        ...
        function highlightBentNotesInPanel(panelId) {
            ...
            console.log('highlightBentNotesInPanel called for panel:', panelId, 'panel element:', panel);
            if (!panel) return;

            // Find all bent lines within this specific panel
            const bentLines = panel.querySelectorAll('.bent-line');
            console.log('Found bent lines:', bentLines.length);
            bentLines.forEach(line => {
                // Get the line's end point coordinates to find the associated note head
                const x2 = parseFloat(line.getAttribute('x2'));
                const y2 = parseFloat(line.getAttribute('y2'));

                // Find the note head circle at those coordinates within the panel
                const noteHead = panel.querySelector(\`circle[cx="\${x2}"][cy="\${y2}"]\`);
                if (noteHead && !noteHead.classList.contains('grace-note')) {
                    // Store original colors using data attributes
                    if (!noteHead.classList.contains('bent-highlighted')) {
                        noteHead.dataset.originalFill = noteHead.style.fill || getComputedStyle(noteHead).fill || '#666666';
                        noteHead.dataset.originalStroke = noteHead.style.stroke || getComputedStyle(noteHead).stroke || '#333333';
                        noteHead.dataset.originalStrokeWidth = noteHead.style.strokeWidth || getComputedStyle(noteHead).strokeWidth || '1';
                        noteHead.setAttribute('data-is-bent', 'true');
                        console.log('Setting data-is-bent on note at', x2, y2);
                    }
                    // Apply red highlight with black outline
                    noteHead.style.fill = '#E74C3C';
                    noteHead.style.stroke = '#000000';
                    noteHead.style.strokeWidth = '2';
                    noteHead.classList.add('bent-highlighted');
                }

                // Find the associated resonance triangle and highlight it
                const resonanceTriangle = panel.querySelector(\`polygon[data-note-x="\${x2}"][data-note-y="\${y2}"]\`);
                if (resonanceTriangle) {
                    if (!resonanceTriangle.dataset.originalFill) {
                        resonanceTriangle.dataset.originalFill = resonanceTriangle.style.fill || '#3498DB';
                        resonanceTriangle.setAttribute('data-is-bent', 'true');
                        console.log('Setting data-is-bent on triangle at', x2, y2);
                    }
                    resonanceTriangle.style.fill = '#E74C3C';
                }
            });
        }
        ...
        function unhighlightBentNotesInPanel(panelId) {
            const panel = document.getElementById(panelId);
            console.log('unhighlightBentNotesInPanel called for panel:', panelId, 'panel element:', panel);
            if (!panel) return;

            // Find all bent notes that were highlighted (marked with data-is-bent)
            const bentNotes = panel.querySelectorAll('circle[data-is-bent="true"]');
            console.log('Found bent notes to unhighlight:', bentNotes.length);

            bentNotes.forEach(circle => {
                // Restore original colors from data attributes
                const originalFill = circle.dataset.originalFill || '#666666';
                const originalStroke = circle.dataset.originalStroke || '#333333';
                const originalStrokeWidth = circle.dataset.originalStrokeWidth || '1';

                console.log('Restoring circle to original fill:', originalFill);
                circle.style.fill = originalFill;
                circle.style.stroke = originalStroke;
                circle.style.strokeWidth = originalStrokeWidth;
                circle.classList.remove('bent-highlighted');

                // Clean up data attributes
                delete circle.dataset.originalFill;
                delete circle.dataset.originalStroke;
                delete circle.dataset.originalStrokeWidth;
                circle.removeAttribute('data-is-bent');
            });

            // Restore resonance triangles
            const bentTriangles = panel.querySelectorAll('polygon[data-is-bent="true"]');
            console.log('Found bent triangles to unhighlight:', bentTriangles.length);

            bentTriangles.forEach(triangle => {
                const originalFill = triangle.dataset.originalFill || '#3498DB';
                console.log('Restoring triangle to original fill:', originalFill);
                triangle.style.fill = originalFill;

                // Clean up data attributes
                delete triangle.dataset.originalFill;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (savedTheme === 'black') {
                ...
                // Ensure white theme is properly set as default (no class needed)
            ...
            setTimeout(() => {
                ...
                if (defaultZoom.x >= 1 && defaultZoom.x < 200 && defaultZoom.y >= 1 && defaultZoom.y < 200) { // SAFE AUTO-ZOOM with font preservation
                    ...
                    const xSlider = document.querySelector('.x-zoom-slider');
                    const ySlider = document.querySelector('.y-zoom-slider');
                    ...
                    if (xSlider) {
                        ...
                        updateZoomDisplay('x', defaultZoom.x);
                    ...
                    if (ySlider) {
                        ...
                        updateZoomDisplay('y', defaultZoom.y);
        ...
        // Enhanced panel toggle to handle analysis panel
        function togglePanel(panelType) {
            let content, indicator;

            if (panelType === 'analysis') {
                content = document.getElementById('analysisPanelContent');
                indicator = document.getElementById('analysisCollapse');
            } else {
                content = document.getElementById(panelType + 'Content');
                indicator = content ? content.previousElementSibling.querySelector('.collapse-indicator') : null;
            }
