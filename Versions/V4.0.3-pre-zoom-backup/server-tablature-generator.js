...
class ServerTablatureGenerator {
    ...
    generateSVG(songData, tuningKey, showBentNotes = true) {
        ...
        // Calculate SVG width based on last note position
        const lastNote = positionedNotes[positionedNotes.length - 1];
        const width = lastNote ? lastNote.x + 200 : 2000;
        const height = 800;
...
${this.generateLyrics(notes)}
    ...
    generateNotesWithTriangles(strings, notes, showBentNotes) {
        ...
        notes.forEach((note, index) => {
            ...
            svg += `    <polygon points="${startX},${topY} ${startX},${bottomY} ${endX},${y}" class="${triangleClass}"/>\n`;
            ...
            if (isBent) {
                ...
                if (nearestString) {
                    ...
                    svg += `    <text x="${dotX}" y="${dotY + 4}" class="bent-indicator bent-elements" text-anchor="middle">●</text>\n`;
                    svg += `    <line x1="${dotX}" y1="${dotY}" x2="${x}" y2="${y}" class="bent-line bent-elements"/>\n`;
                }
            }

            // Note circle
            const circleClass = isGrace ? 'grace-note' : (isBent ? 'bent-note' : 'note-circle');
            svg += `    <circle cx="${x}" cy="${y}" r="${noteRadius}" class="${circleClass}"/>\n`;
