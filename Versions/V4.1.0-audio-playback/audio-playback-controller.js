...
class AudioPlaybackController {
    ...
    /**
     * Initialize audio context and button references
     */
    initialize() {
        ...
        // Find button references (will be created in template)
        ...
        if (this.stopButton) {
            ...
            this.stopButton.disabled = true; // Disabled until playback starts
        }
    ...
    setupNoteClickHandlers() {
        ...
        this.notes.forEach((note, index) => {
            ...
            note.element.style.cursor = 'pointer';
        });
    }
    ...
    playNoteWithVisuals(note, duration) {
        ...
        setTimeout(() => {
            this.removeNoteGlow(note);
        }, duration);
    }

    /**
     * Add red glow and animation to note (in both tablatures)
     */
    addNoteGlow(note) {
        ...
        // Find corresponding elements in both tablatures
        ...
        Object.values(this.svgElements).forEach(svg => {
            ...
            if (element) {
                ...
                // Change fill to red using inline style (overrides CSS)
    ...
    /**
     * Calculate note duration in milliseconds based on note value
     */
    calculateNoteDuration(noteValue) {
        // noteValue: 1 = quarter note, 0.5 = eighth note, 2 = half note, etc.
        return this.quarterNoteDuration * noteValue;
    }
