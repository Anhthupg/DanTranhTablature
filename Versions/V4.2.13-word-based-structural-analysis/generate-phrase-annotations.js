...
class PhraseAnnotationsGenerator {
    ...
    getSectionLabel(sectionType, sectionNumber) {
        const labels = {
            'intro': 'INTRO',
            'verse': `VERSE ${Math.floor(sectionNumber / 2)}`,
            'refrain': `REFRAIN ${Math.floor((sectionNumber + 1) / 2)}`,
            'dialogue': `DIALOGUE ${sectionNumber}`,
            'coda': 'CODA'
        };
        return labels[sectionType] || `SECTION ${sectionNumber}`;
    }
