...
app.get('/', (req, res) => {
    ...
    // V4.2.7: Default to "Bà rằng bà rí" for phrase annotation demo
    const v3ProcessedDir = path.join(__dirname, '..', 'v3', 'data', 'processed');
    const preferredSong = 'Bà rằng bà rí';
    ...
    if (!fs.existsSync(path.join(v3ProcessedDir, preferredSong, 'relationships.json'))) {
        ...
        songDir = songDirs[0];
        console.log(`Preferred song not found, using: ${songDir}`);
    } else {
        console.log(`Loading preferred song: ${songDir}`);
    }
    ...
    try {
        ...
        if (fs.existsSync(relationshipsPath)) {
            ...
            if (fs.existsSync(lyricsPath)) {
                ...
                // Generate structural overview cards (simple HTML) - assign to outer variable
                structuralOverviewCards = analysis.sections.map(section => {
                    const typeColors = {
                        intro: '#3498DB',
                        verse: '#9B59B6',
                        refrain: '#F39C12',
                    ...
                    const color = typeColors[section.type] || '#95A5A6';
