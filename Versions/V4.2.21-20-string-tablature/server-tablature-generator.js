...
${this.generateLyrics(notes)}
    ...
    generateNotesWithTriangles(strings, notes, showBentNotes) {
        ...
        notes.forEach((note, index) => {
            ...
            svg += `    <circle id="${noteId}" cx="${x}" cy="${y}" r="${noteRadius}" class="${circleClass}"${bentAttr}${graceTypeAttr} data-pitch="${pitch}" data-duration="${note.duration || 1}" data-note-index="${index}"/>\n`;
            ...
            // Find which string this note is on (for string number mode)
            const stringNumber = this.findStringNumberForNote(strings, note.y);
     ...
     */
    /**
     * Generate glissando markers - simple duration-based arcs
     * All notes get arcs, color-coded by relative duration
     */
    generateGlissandoPaths(notes, glissandoCandidates) {
        if (!Array.isArray(glissandoCandidates) || glissandoCandidates.length === 0) {
            return '';
        }

        const markers = [];

        glissandoCandidates.forEach((candidate, index) => {
            // Find the note in positioned notes
            const note = notes.find(n => n.index === candidate.noteIndex);
            if (!note) return;

            const noteRadius = note.isGrace ? 6 : 12;

            // Add a colored arc above the note
            const arcRadius = noteRadius + 8;
            const arcX = note.x;
            const arcY = note.y;

            // Create arc path (semi-circle above note)
            const startX = arcX - arcRadius;
            const startY = arcY;
            const endX = arcX + arcRadius;
            const endY = arcY;
            const largeArcFlag = 1;

            const pathData = `M ${startX},${startY} A ${arcRadius},${arcRadius} 0 ${largeArcFlag} 1 ${endX},${endY}`;

            // Color by duration value (gold for longest)
            const color = index === 0 ? '#FFD700' : (index < glissandoCandidates.length / 3 ? '#3498db' : '#95a5a6');

            markers.push(`    <path d="${pathData}" class="glissando-marker" data-glissando="true" data-note-index="${candidate.noteIndex}" data-duration="${candidate.duration}" stroke="${color}" stroke-width="3" fill="none"/>`);
        });
