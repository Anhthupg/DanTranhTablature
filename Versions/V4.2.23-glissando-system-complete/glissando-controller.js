...
class GlissandoController {
    ...
    extractStringsFromSVG() {
        ...
        stringLines.forEach((line, index) => {
            ...
            console.log(`  String ${stringNum}: Y=${y}, stroke=${stroke}`);
...
╚═══════════════════════════════════════════════════════════════╝
            ...
            console.log(`Total strings: ${this.allStrings.length} (${this.strings.length} used black, ${this.allStrings.length - this.strings.length} unused grey)`);
            console.log(`All strings:`, this.allStrings.map(s => `String ${s.number} (y=${s.y}, ${s.isUsed ? 'BLACK' : 'GREY'})`));

            // Add visual labels on tablature for ABSOLUTE boundaries (including grey strings)
            const topLabel = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            topLabel.setAttribute('x', '10');
            topLabel.setAttribute('y', absoluteTopString.y);
            topLabel.setAttribute('fill', absoluteTopString.isUsed ? '#0000FF' : '#FF00FF'); // Blue for used, Magenta for grey
            topLabel.setAttribute('font-size', '14');
            topLabel.setAttribute('font-weight', 'bold');
            topLabel.setAttribute('dominant-baseline', 'middle');
            topLabel.textContent = `TOP: Y=${absoluteTopString.y} ${absoluteTopString.isUsed ? '' : '(GREY)'}`;
            topLabel.classList.add('string-boundary-label');

            const bottomLabel = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            bottomLabel.setAttribute('x', '10');
            bottomLabel.setAttribute('y', absoluteBottomString.y);
            bottomLabel.setAttribute('fill', absoluteBottomString.isUsed ? '#0000FF' : '#FF00FF'); // Blue for used, Magenta for grey
            ...
            bottomLabel.textContent = `BOTTOM: Y=${absoluteBottomString.y} ${absoluteBottomString.isUsed ? '' : '(GREY)'}`;
            bottomLabel.classList.add('string-boundary-label');
    ...
    drawGlissandoForCandidate(candidateNoteIndex) {
        ...
        // Create Y label at start of glissando
        const startLabel = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        startLabel.setAttribute('x', xFrom - 30);
        startLabel.setAttribute('y', yFrom);
        startLabel.setAttribute('fill', '#FF0000');
        startLabel.setAttribute('font-size', '16');
        startLabel.setAttribute('font-weight', 'bold');
        startLabel.setAttribute('dominant-baseline', 'middle');
        startLabel.textContent = `Y=${yFrom.toFixed(0)}`;
        startLabel.classList.add('glissando-label');
        ...
        // Insert label and chevrons into SVG (before notes for proper layering)
        const firstNote = this.svg.querySelector('circle');
        this.svg.insertBefore(startLabel, firstNote);
        chevrons.forEach(chevron => {
            this.svg.insertBefore(chevron, firstNote);
        });

        // Store path data (include label for removal)
    ...
    clearAllGlissandos() {
        this.glissandoPaths.forEach(path => {
            ...
            if (path.label) path.label.remove();
        });
    ...
    removeGlissandoForNote(noteIndex) {
        ...
        // Remove chevrons and label from DOM
