...
class NoteLyricsService {
        ...
        try {
            ...
            const annotatedNotes = notes.map(note => {
                ...
                return {
                    ...
                    tone: this.extractTone(noteIdMap[note.id]?.lyric) || null,  // Extract from lyric
                    rhymeFamily: this.extractRhymeFamily(noteIdMap[note.id]?.lyric) || null,  // Extract from lyric
                    id: note.id
                };
            });
     ...
     * @param {string} syllable - Vietnamese syllable with tone marks
     * @returns {string|null} Tone name (ngang, sắc, huyền, hỏi, ngã, nặng)
     */
    extractTone(syllable) {
        if (!syllable) return null;

        // Normalize to NFD (decomposed form) to detect diacritics
        const normalized = syllable.normalize('NFD');

        // Check for tone marks (combining diacritics)
        if (/\u0301/.test(normalized)) return 'sắc';      // Acute accent (́)
        if (/\u0300/.test(normalized)) return 'huyền';    // Grave accent (̀)
        if (/\u0309/.test(normalized)) return 'hỏi';      // Hook above (̉)
        if (/\u0303/.test(normalized)) return 'ngã';      // Tilde (̃)
        if (/\u0323/.test(normalized)) return 'nặng';     // Dot below (̣)

        return 'ngang';  // Level tone (no marks)
    }

    /**
     * Extract rhyme family from syllable final sound
     * @param {string} syllable - Vietnamese syllable
     * @returns {string|null} Rhyme family (exact match with pattern data)
     */
    extractRhymeFamily(syllable) {
        if (!syllable) return null;

        // Remove tone marks and normalize
        const normalized = syllable.normalize('NFD').replace(/[\u0300-\u036f]/g, '').normalize('NFC').toLowerCase();

        // Extract final rhyme including consonants (more accurate)
        // Match vowel + optional consonants: ông, ăng, ên, anh, etc.
        const rhymeMatch = normalized.match(/[aeiouăâêôơưy]+[nptcmkghnhch]*$/i);
        if (!rhymeMatch) {
            // Try final consonant only
            const consonantMatch = normalized.match(/[bcdfghjklmnpqrstvwxyz]+$/i);
            return consonantMatch ? `${consonantMatch[0]}-family` : 'other-family';
        }

        const rhyme = rhymeMatch[0];

        // Return exact rhyme with -family suffix to match pattern data format
        return `${rhyme}-family`;
    }
}
