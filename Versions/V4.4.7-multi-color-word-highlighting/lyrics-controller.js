...
class LyricsController {
    constructor() {
        ...
        this.isLooping = {};
        this.highlightedWords = new Map(); // Changed to Map to store word -> color mapping
        ...
        // Word highlighting color palette (20 vibrant colors)
        this.wordColors = [
            '#FF6B6B', // Red
            '#4ECDC4', // Turquoise
            '#45B7D1', // Sky blue
        ...
        this.colorIndex = 0; // Track which color to use next
    }
    ...
    renderLyricsTable() {
        ...
        const generateClickableWords = (phrase, phraseIndex) => {
            ...
            return phrase.wordMapping.map((m, idx) =>
                `<span class="syllable" data-phrase-id="${phrase.id}" data-word-index="${idx}" data-vn="${m.vn}" data-en="${m.en}"
                 style="cursor: pointer; padding: 2px 4px; border-radius: 3px; transition: all 0.2s; border: 2px solid transparent;">${m.vn}</span>`
            ).join(' ');
        };
        ...
        const generateClickableEnglish = (phrase, phraseIndex) => {
            ...
            return phrase.wordMapping.map((m, idx) =>
                `<span class="translation-word" data-phrase-id="${phrase.id}" data-word-index="${idx}" data-vn="${m.vn}" data-en="${m.en}"
                 style="cursor: pointer; padding: 2px 4px; border-radius: 3px; transition: all 0.2s; border: 2px solid transparent;">${m.en}</span>`
            ).join(' ');
        };
                ...
                <tr data-phrase="${i}" data-phrase-id="${phrase.id}" style="border-bottom: 1px solid #e0e0e0; transition: background 0.2s;">
                    ...
                    <td style="padding: 4px; text-align: center; vertical-align: middle;">
                        <div style="display: flex; gap: 3px; justify-content: center; flex-wrap: nowrap;">
                            ...
                            <button title="Cultural Context" onclick="if(window.lyricsController) window.lyricsController.showCulturalContext(${phrase.id})" style="width: 22px; height: 22px; border: 1px solid #F39C12; background: white; border-radius: 3px; cursor: pointer; font-size: 10px; padding: 0; color: #F39C12;">📚</button>
                            <button title="Google Translate" onclick="if(window.lyricsController) window.lyricsController.openGoogleTranslate('${phrase.text.replace(/'/g, "\\'")}', '${(phrase.english || '').replace(/'/g, "\\'")}', ${phrase.id})" style="width: 22px; height: 22px; border: 1px solid #4285F4; background: white; border-radius: 3px; cursor: pointer; font-size: 10px; padding: 0; color: #4285F4;">🌐</button>
                            <button title="Google Voice" onclick="if(window.lyricsController) window.lyricsController.playGoogleVoice('${phrase.text.replace(/'/g, "\\'")}', ${phrase.id})" style="width: 22px; height: 22px; border: 1px solid #34A853; background: white; border-radius: 3px; cursor: pointer; font-size: 10px; padding: 0; color: #34A853;">🔊</button>
                        </div>
                    </td>
            ...
            <table id="lyricsTable" style="width: auto; border-collapse: collapse; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); table-layout: fixed;">
                <thead>
                    <tr style="background: #008080; color: white;">
                        ...
                        <th style="width: 180px; padding: 8px; text-align: center; font-size: 11px;">Controls</th>
    ...
    async loadLyricsData() {
        try {
            ...
            console.log(`[LyricsController] Loaded ${this.lyricsData.phrases.length} phrases for "${currentSong}"`);
        } catch (error) {
    ...
    async loadRelationships() {
        try {
            ...
            console.log(`[LyricsController] Loaded relationships with ${this.relationships.wordToNoteMap.length} word mappings for "${currentSong}"`);
        } catch (error) {
    ...
    handleWordClick(event, row) {
        ...
        if (this.highlightedWords.has(wordKey)) {
            ...
            console.log(`[LyricsController] Cleared highlight for "${vnWord}" → "${enWord}"`);
        } else {
            // Add new highlight with unique color
            const color = this.wordColors[this.colorIndex % this.wordColors.length];
            this.colorIndex++;

            this.highlightWord(phraseId, wordIndex, color);
            this.highlightedWords.set(wordKey, color);
            console.log(`[LyricsController] Highlighted "${vnWord}" → "${enWord}" with color ${color}`);
        }
    ...
    highlightWord(phraseId, wordIndex, color) {
        const row = document.querySelector(`tr[data-phrase-id="${phraseId}"]`);
        if (!row) return;

        // Highlight both Vietnamese and English words with the same color
        ...
        if (vnWord) {
            vnWord.style.backgroundColor = color;
            vnWord.style.color = 'white';
            vnWord.style.fontWeight = 'bold';
            vnWord.style.borderColor = this.darkenColor(color, 20);
            vnWord.style.transform = 'scale(1.05)';
        }

        if (enWord) {
            enWord.style.backgroundColor = color;
            enWord.style.color = 'white';
            enWord.style.fontWeight = 'bold';
            enWord.style.borderColor = this.darkenColor(color, 20);
            enWord.style.transform = 'scale(1.05)';
        }
    }
    ...
    clearWordHighlight(phraseId, wordIndex) {
        ...
        if (vnWord) {
            vnWord.style.backgroundColor = '';
            vnWord.style.color = '';
            vnWord.style.fontWeight = '';
            vnWord.style.borderColor = 'transparent';
            vnWord.style.transform = '';
        }

        if (enWord) {
            enWord.style.backgroundColor = '';
            enWord.style.color = '';
            enWord.style.fontWeight = '';
            enWord.style.borderColor = 'transparent';
            enWord.style.transform = '';
        }
    }
    ...
    clearAllHighlights() {
        console.log('[LyricsController] Clearing all word highlights');

        // Clear all highlighted words
        this.highlightedWords.forEach((color, wordKey) => {
            const [phraseId, wordIndex] = wordKey.split('-');
            this.clearWordHighlight(phraseId, wordIndex);
        });

        // Reset the map and color index
        this.highlightedWords.clear();
        this.colorIndex = 0;

        console.log('[LyricsController] All highlights cleared');
    }
    ...
    /**
     * Show Cultural Context for a phrase
     */
    showCulturalContext(phraseId) {
        const phrase = this.lyricsData.phrases.find(p => p.id === phraseId);
        if (!phrase) {
            console.error('[LyricsController] Phrase not found:', phraseId);
            return;
        }
        ...
        // Get cultural context from phrase data or generate default
        const culturalContext = phrase.culturalContext || this.generateCulturalContext(phrase);

        // Build content
        let content = `
            <div style="border-bottom: 2px solid #f39c12; padding-bottom: 12px; margin-bottom: 15px;">
                <h3 style="margin: 0; color: #2c3e50;">Cultural Context - Phrase ${phrase.id}</h3>
            </div>

            <div style="margin-bottom: 20px;">
                <div style="font-weight: 600; color: #7f8c8d; font-size: 11px; text-transform: uppercase; margin-bottom: 5px;">Vietnamese Text</div>
                <div style="font-size: 20px; color: #2c3e50; margin-bottom: 10px; font-weight: 600;">${phrase.text}</div>
            ...
            <div style="background: #fff3e0; padding: 15px; border-radius: 8px; border-left: 4px solid #f39c12; margin-bottom: 15px;">
                <div style="font-weight: 600; color: #2c3e50; margin-bottom: 10px; font-size: 15px;">Cultural Context</div>
                <div style="color: #555; line-height: 1.7; font-size: 14px;">
                    ${culturalContext.description}
                </div>
            </div>

            ${culturalContext.facts && culturalContext.facts.length > 0 ? `
                <div style="background: #e8f5e9; padding: 15px; border-radius: 8px; border-left: 4px solid #27ae60; margin-bottom: 15px;">
                    <div style="font-weight: 600; color: #2c3e50; margin-bottom: 10px; font-size: 15px;">Vietnamese Cultural Facts</div>
                    <ul style="margin: 5px 0; padding-left: 20px; color: #555; font-size: 14px; line-height: 1.7;">
                        ${culturalContext.facts.map(fact => `<li>${fact}</li>`).join('')}
                    </ul>
                </div>
            ` : ''}

            ${culturalContext.musicalContext ? `
                <div style="background: #e3f2fd; padding: 15px; border-radius: 8px; border-left: 4px solid #3498db; margin-bottom: 15px;">
                    <div style="font-weight: 600; color: #2c3e50; margin-bottom: 10px; font-size: 15px;">Musical Context</div>
                    <div style="color: #555; line-height: 1.7; font-size: 14px;">
                        ${culturalContext.musicalContext}
                    </div>
                </div>
            ...
            <div style="margin-top: 20px; text-align: right;">
                <button onclick="this.closest('.modal-container').closeModal()" style="background: #e74c3c; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-size: 14px;">
                    Close
                </button>
            </div>
        `;
        ...
        console.log(`[LyricsController] Showing cultural context for phrase ${phraseId}`);
    }

    /**
     * Generate cultural context for a phrase
     */
    generateCulturalContext(phrase) {
        // Default cultural context based on phrase content
        // In production, this would come from a database or LLM
        return {
            description: `This phrase is part of traditional Vietnamese folk music, expressing themes common in Northern Vietnamese culture. The linguistic type "${phrase.linguisticType}" indicates its role in the song's emotional narrative.`,
            facts: [
                'Vietnamese folk songs often use call-and-response patterns between phrases',
                'Traditional Dan Tranh music preserves centuries-old cultural expressions',
                'Many phrases contain references to nature, family, and daily life in rural Vietnam',
                'The tonal language creates a natural musicality that influences the melody'
            ],
            musicalContext: `This phrase contains ${phrase.syllableCount} syllables and follows traditional Vietnamese musical phrasing patterns. The melody typically follows the natural tones of the Vietnamese language.`
        };
    }

    /**
     * Open Google Translate for a phrase
     */
    openGoogleTranslate(text, translation, phraseId) {
        const encodedText = encodeURIComponent(text);
        const googleTranslateUrl = `https://translate.google.com/?sl=vi&tl=en&text=${encodedText}&op=translate`;

        window.open(googleTranslateUrl, '_blank', 'width=800,height=600');

        console.log(`[LyricsController] Opened Google Translate for phrase ${phraseId}: "${text}"`);
    }

    /**
     * Play phrase using Google Voice (Web Speech API)
     */
    playGoogleVoice(text, phraseId) {
        console.log(`[LyricsController] Playing Google Voice for phrase ${phraseId}: "${text}"`);
