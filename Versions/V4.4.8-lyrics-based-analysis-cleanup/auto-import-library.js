...
class V4LibraryManager {
    ...
    extractMetadata(musicXMLPath) {
        try {
            ...
            const title = this.extractTitle(xmlDoc, filename);
     ...
     */
    extractTitle(xmlDoc, filename = null) {
        ...
        if (filename) {
            return filename.replace(/\.(musicxml\.xml|musicxml|xml)$/i, '');
        }
    ...
    /**
     * Recursively scan for MusicXML files in all subfolders
     */
    scanMusicXMLFilesRecursively(dirPath, category = null) {
        const results = [];
        ...
        items.forEach(item => {
            ...
            if (stat.isDirectory()) {
                ...
                const subResults = this.scanMusicXMLFilesRecursively(fullPath, item);
                results.push(...subResults);
            } else if (stat.isFile() && (item.toLowerCase().endsWith('.xml') || item.toLowerCase().endsWith('.musicxml'))) {
                // Found a MusicXML file
                results.push({
                    filename: item,
                    filePath: fullPath,
                    category: category
    ...
    scanAndUpdateLibrary() {
        ...
        // Scan for MusicXML files (recursively in all subfolders)
        const files = this.scanMusicXMLFilesRecursively(this.musicXMLDir);

        let newFiles = 0;
        files.forEach(({ filename, filePath, category }) => {
            if (!existingFiles.has(filename)) {
                ...
                if (metadata) {
                    ...
                    metadata.categoryPath = category ? `${category}/${filename}` : filename;

                    library.push(metadata);
                    newFiles++;
                    console.log(`Added: ${metadata.title} [${category || 'root'}] (${metadata.region}, ${metadata.genre})`);
                }
            }
        });
