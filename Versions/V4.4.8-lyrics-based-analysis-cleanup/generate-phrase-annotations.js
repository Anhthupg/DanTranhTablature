...
class PhraseAnnotationsGenerator {
    ...
    detectSections(phrases, exactRefrains) {
        // Reset section counters for each song (MUST match getSectionLabel!)
    ...
    determineSectionType(phrase, isRefrain, index, totalPhrases) {
        // Special positions (structural)
        if (index === 0) return 'intro';
        if (index === totalPhrases - 1) return 'coda';

        // ✅ PURE LINGUISTIC TYPES - No mixing with repetition!
        // Repetition is shown by REPEAT badge (Layer 2), not section type

        if (phrase.linguisticType === 'exclamatory') return 'exclamatory';
        ...
        if (phrase.linguisticType === 'imperative') return 'imperative';
        if (phrase.linguisticType === 'vocative') return 'vocative';
    ...
    getSectionLabel(sectionType, sectionNumber) {
        ...
        const labels = {
            ...
            // Pure linguistic types
            'exclamatory': () => `Exclamatory ${++this.sectionCounters.exclamatory}`,
            'question': () => `Question ${++this.sectionCounters.question}`,
            'answer': () => `Answer ${++this.sectionCounters.answer}`,
            'complaint': () => `Complaint ${++this.sectionCounters.complaint}`,
            'narrative': () => `Narrative ${++this.sectionCounters.narrative}`,
            'imperative': () => `Imperative ${++this.sectionCounters.imperative}`,
            'vocative': () => `Vocative ${++this.sectionCounters.vocative}`
        };
        ...
        if (exactRefrain) {
            ...
            return {
                ...
                badge: {
                    ...
                    badgeClass: 'exact',
                    color: '#D35400'  // Dark orange - readable on all light backgrounds
                }
            };
        }
        ...
        if (structuralMatch && structuralMatch.phraseIds.length > 1) {
            return {
                ...
                badge: {
                    ...
                    color: '#2C3E50'  // Dark blue-grey - readable on all light backgrounds
                }
            };
        }
    ...
    /**
     * Detect figurative language patterns (NEW: V4.4.9)
     * Identifies when nature imagery is used to express emotions/abstract concepts
     */
    detectFigurativeLanguage(phrase) {
        const themes = this.extractThemes(phrase.wordMapping);

        const hasNature = themes.includes('nature');
        const hasEmotion = themes.includes('emotions');
        const hasAbstract = themes.includes('abstract');

        // Metaphor: Nature + Emotion/Abstract (borrowing nature to talk about feeling)
        if (hasNature && (hasEmotion || hasAbstract)) {
            return {
                type: 'metaphor',
                pattern: 'nature-emotion',
                icon: '🎭',
                description: 'Uses nature to express emotion'
            };
        }

        // Pure emotion (literal emotional expression)
        ...
        if (hasNature && !hasEmotion && !hasAbstract) {
            return {
                type: 'literal-nature',
                icon: '🌿',
                description: 'Pure nature description'
            };
        }
     ...
     * ✅ V4.4.9: Only shows NON-REDUNDANT, NON-OBVIOUS info (structural role only)
     *
     * Removed redundancies:
     * - OPENING/CLOSING (already shown by Intro/Coda sections)
     * - Linguistic type labels (already shown in section names)
     */
    getFunctionLabel(phrase, index, totalPhrases) {
        // Structural role from V4.4.8 migration (adds info beyond linguistic type)
        if (phrase.structuralRole === 'refrain') {
            return {
                text: 'HOOK',
                tooltip: 'Structural anchor phrase - memorable repeated element that defines the song'
            };
        }
        ...
        if (phrase.structuralRole === 'bridge') {
            return {
                text: 'CONTRAST',
                tooltip: 'Contrasting section - breaks the pattern to add variety'
            };
        }

        // No label for most phrases!
        // - Position already shown by Intro/Coda sections
        // - Linguistic type already shown in section name
        // - Only HOOK and CONTRAST add non-obvious structural info
        return null;
    }
