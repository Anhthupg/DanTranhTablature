...
module.exports = function(app, baseDir, templateComposer) {
    ...
    const annotatedPhrasesIIGen = new AnnotatedPhrasesIIGenerator();
    ...
    app.get('/', (req, res) => {
        try {
            ...
            // 5.5. Generate Multi-Dimensional Phrase Analysis (4-layer linguistic-musical system)
            let annotatedPhrasesIISVG = '';
            let annotatedIISvgWidth = 2000;
            let annotatedIISvgHeight = 800;

            if (relationshipsData && segmentationResult && tablatures.optimalPositionedNotes) {
                try {
                    const phrasesIIResult = annotatedPhrasesIIGen.generate(
                        segmentationResult,
                        relationshipsData,
                        tablatures.optimalPositionedNotes
                    );
                    ...
                    if (phrasesIIResult) {
                        annotatedPhrasesIISVG = phrasesIIResult.svg;
                        annotatedIISvgWidth = phrasesIIResult.width;
                        annotatedIISvgHeight = phrasesIIResult.height;
                        console.log(`[AnnotatedPhrasesII] Generated ${phrasesIIResult.phraseCount} phrase boxes (${phrasesIIResult.width}x${phrasesIIResult.height})`);
                    }
                } catch (error) {
                    console.error('[AnnotatedPhrasesII] Generation error:', error);
                }
            }
            ...
            const html = templateComposer.render('v4-vertical-header-sections-annotated.html', {
                ...
                SYLLABLE_LYRICS_HTML: syllableLyricsHTML,  // V4.3.5+ syllable lyrics with X positions
                ANNOTATED_PHRASES_II_SVG: annotatedPhrasesIISVG,  // Multi-Dimensional Phrase Analysis (4-layer system)
                ANNOTATED_II_SVG_WIDTH: annotatedIISvgWidth,
                ANNOTATED_II_SVG_HEIGHT: annotatedIISvgHeight
            });
