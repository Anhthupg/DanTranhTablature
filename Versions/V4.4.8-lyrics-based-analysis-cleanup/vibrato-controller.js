...
class VibratoController {
    constructor() {
        ...
        // Pitch class color mapping (deep, saturated chromatic rainbow)
        this.pitchClassColors = {
            'C': '#D90606',    'C#': '#D94906',   'Db': '#D94906',
            'D': '#D98C06',    'D#': '#CCD906',   'Eb': '#CCD906',
            'E': '#89D906',    'F': '#46D906',
            'F#': '#06D946',   'Gb': '#06D946',
            'G': '#06D989',    'G#': '#06CCD9',   'Ab': '#06CCD9',
            'A': '#0689D9',    'A#': '#0646D9',   'Bb': '#0646D9',
            'B': '#8906D9'
        };
    }
    ...
    createVibratoUI(containerId, pitchClasses) {
        ...
        title.style.color = '#008080'; // Teal to match Glissando Á
        ...
        tapTitle.style.color = '#008080'; // Teal to match Glissando Á
        ...
        Object.entries(notesByString).forEach(([stringNum, stringNotes]) => {
            ...
            stringNotes.forEach((note, index) => {
                ...
                if (endX > startX) {
                    ...
                    // Get pitch-specific color
                    const pitchColor = this.pitchClassColors[note.pitchClass] || '#FF0000';
                    ...
                    const vibratoPath = this.vibratoGenerator.createVibratoPath(
                        ...
                        note.y,
                        {
                            ...
                            color: pitchColor  // Use pitch-specific color
                        }
                    );
                    ...
                    console.log(`Vibrato path inserted into SVG with color ${pitchColor}`);
                }
            });
        });
    }
    ...
    updateTaps() {
        ...
        validDottedNotes.forEach(note => {
            ...
            chevron.setAttribute('data-tap-position', this.tapPosition);
            chevron.setAttribute('fill', 'none');

            // Color wheel progression (opposite of red glissando warm colors)
            // Cool side of color wheel: Blue → Cyan → Green
            const tapColors = {
                0: '#0066FF',        // Position 0: Blue (opposite of red)
                [1/3]: '#00CCCC',    // Position 1/3: Cyan (between blue and green)
                [2/3]: '#00CC00'     // Position 2/3: Green (completes cool progression)
            };
            const selectedColor = tapColors[this.tapPosition] || '#0055DD';
            console.log(`TAP COLOR DEBUG: position=${this.tapPosition}, color=${selectedColor}`);

            // Use inline styles to override CSS
            chevron.style.stroke = selectedColor;
            chevron.style.strokeWidth = '4';
            chevron.style.strokeLinecap = 'round';
            chevron.style.strokeLinejoin = 'round';
