...
class ZoomController {
    ...
    initialize() {
        ...
        this.sections.forEach(section => {
            ...
            // Different container class for annotated sections
            let containerSelector = '.tablature-reference';
            if (section === 'annotated' || section === 'annotatedII') {
                containerSelector = '.tablature-scroll-container';
            }
    ...
    transformElements(svg, xZoom, yZoom) {
        ...
        svg.querySelectorAll('text[data-base-x]').forEach(text => {
            ...
            if (!isNaN(baseX)) {
                // Check if this text belongs to a phrase annotation group
                const phraseGroup = text.closest('g.phrase-annotation');
                const sectionGroup = text.closest('g.section-group');

                if (phraseGroup) {
                    // TEXT INSIDE PHRASE BOX: Recalculate center from scaled rect
                    const phraseRect = phraseGroup.querySelector('rect');
                    if (phraseRect) {
                        const rectX = parseFloat(phraseRect.getAttribute('x'));
                        const rectWidth = parseFloat(phraseRect.getAttribute('width'));
                        const scaledCenterX = rectX + (rectWidth / 2);
                        text.setAttribute('x', scaledCenterX);
                    } else {
                        // Fallback to pivot scaling
                        const scaledX = 60 + (baseX - 60) * xZoom;
                        text.setAttribute('x', scaledX);
                    }
                } else if (sectionGroup) {
                    // TEXT INSIDE SECTION BOX: Recalculate center from scaled section rect
                    const sectionRect = sectionGroup.querySelector('rect');
                    if (sectionRect) {
                        const rectX = parseFloat(sectionRect.getAttribute('x'));
                        const rectWidth = parseFloat(sectionRect.getAttribute('width'));
                        const scaledCenterX = rectX + (rectWidth / 2);
                        text.setAttribute('x', scaledCenterX);
                    } else {
                        // Fallback to pivot scaling
                    ...
                    // TEXT OUTSIDE ANY BOX: Use standard pivot scaling
                    const scaledX = 60 + (baseX - 60) * xZoom;
                    text.setAttribute('x', scaledX);
                }
            }
        ...
        // V4.2.14: Transform <g> elements with translate (semantic icon groups)
        svg.querySelectorAll('g[data-base-x]').forEach(group => {
            const baseX = parseFloat(group.getAttribute('data-base-x') || group.dataset.baseX);

            if (!isNaN(baseX)) {
                // Check if this group is inside a phrase annotation
                const phraseGroup = group.closest('g.phrase-annotation');

                if (phraseGroup) {
                    // GROUP INSIDE PHRASE: Recalculate X from scaled rect center
                    const phraseRect = phraseGroup.querySelector('rect');
                    if (phraseRect) {
                        const rectX = parseFloat(phraseRect.getAttribute('x'));
                        const rectWidth = parseFloat(phraseRect.getAttribute('width'));
                        ...
                        // Extract Y from current transform
                        const transform = group.getAttribute('transform');
                        const yMatch = transform ? transform.match(/translate\([^,]+,\s*(\d+)\)/) : null;
                        const y = yMatch ? yMatch[1] : '110';

                        group.setAttribute('transform', `translate(${scaledCenterX}, ${y})`);
                    } else {
                        // Fallback to pivot scaling
                        const scaledX = 60 + (baseX - 60) * xZoom;
                        const transform = group.getAttribute('transform');
                    ...
                    // GROUP OUTSIDE PHRASE: Use standard pivot scaling
                    const scaledX = 60 + (baseX - 60) * xZoom;
                    const transform = group.getAttribute('transform');
                    const yMatch = transform ? transform.match(/translate\([^,]+,\s*(\d+)\)/) : null;
                    const y = yMatch ? yMatch[1] : '110';
