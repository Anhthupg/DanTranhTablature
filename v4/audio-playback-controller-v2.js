...
class AudioPlaybackController {
    ...
    extractNotesFromSVG(svg) {
        if (!svg) {
            console.warn('[AudioController] extractNotesFromSVG: svg is null/undefined');
            return;
        }

        console.log('[AudioController] extractNotesFromSVG: Extracting from SVG:', svg.id);

        this.notes = [];

        // Find all note circles (class="note" or class="grace-note")
        const noteCircles = svg.querySelectorAll('circle.note, circle.grace-note');
        console.log(`[AudioController] Found ${noteCircles.length} circles with .note or .grace-note classes`);
     ...
     * @param {Array<string>} noteIds - Array of note IDs to play (e.g., ['note_0', 'note_1', 'note_2'])
    ...
    playNoteIds(noteIds, mainNotesOnly = false, loop = false) {
        console.log(`[AudioController] Playing note IDs:`, noteIds);
        console.log(`[AudioController] mainNotesOnly: ${mainNotesOnly}, loop: ${loop}`);
        ...
        if (mainNotesOnly) {
            ...
            console.log(`[AudioController] Filtered to ${notesToPlay.length} main notes (excluded grace notes)`);
        }
        ...
        console.log(`[AudioController] Playing ${notesToPlay.length} notes`);
    ...
    stop() {
        ...
        if (window.lyricsController && window.lyricsController.currentlyPlaying) {
            const playingPhraseId = window.lyricsController.currentlyPlaying;
            window.lyricsController.currentlyPlaying = null;

            // Disable loop for that phrase
            if (window.lyricsController.isLooping[playingPhraseId]) {
                window.lyricsController.isLooping[playingPhraseId] = false;

                // Reset loop button visually
                const loopBtn = document.querySelector(`.loop-btn-${playingPhraseId}`);
                if (loopBtn) {
    ...
    schedulePlayback(startIndex) {
        ...
        console.log(`[AudioController] Scheduling ${this.notes.length} notes from index ${startIndex}`);

        if (this.notes.length === 0) {
            console.warn('[AudioController] No notes to schedule!');
            this.stop();
            return;
        }
        ...
        for (let i = startIndex; i < this.notes.length; i++) {
            ...
            const timeout = setTimeout(() => {
                ...
                if (i === this.notes.length - 1) {
                    ...
                    console.log(`[AudioController] Last note played, scheduling stop in ${noteDuration}ms`);
                    setTimeout(() => this.stop(), noteDuration);
                }
            }, cumulativeTime);
        ...
        console.log(`[AudioController] Scheduled ${this.notes.length} notes, total duration: ${cumulativeTime}ms`);
    }
     ...
     * V4.4.2: Scroll BOTH tablatures (optimal + alternative) synchronously
     */
    startContinuousScroll(startIndex) {
        ...
        // Find BOTH tablature containers (optimal + alternative)
        const containers = [];
        ...
        if (optimalContainer) containers.push(optimalContainer);
        if (alt1Container) containers.push(alt1Container);

        if (containers.length === 0) {
            console.warn('[ContinuousScroll] No containers found!');
            return;
        }

        console.log(`[ContinuousScroll] Found ${containers.length} container(s) to scroll`);
        ...
        console.log(`[ContinuousScroll] Scrolling ${containers.length} container(s) synchronously`);

        // Set initial scroll position for ALL containers
        ...
        const animate = () => {
            ...
            // Update scrollLeft for ALL containers
            containers.forEach(container => {
