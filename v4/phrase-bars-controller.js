...
class PhraseBarsController {
    ...
    async loadLyricsData() {
        try {
            ...
            // Strip .musicxml.xml extension if present
    ...
    renderPhraseBars() {
        ...
        for (const phrase of this.lyricsData.phrases) {
            ...
            // Find first and last word mappings
            ...
            if (!firstMapping || !lastMapping) {
                ...
                console.warn(`[PhraseBars]   phraseWords.length: ${phraseWords.length}, wordToNoteMap entries for this phrase: ${this.relationships.wordToNoteMap.filter(m => m.phraseId === phrase.id).length}`);
                continue;
            }

            // Get note positions from SVG
            // V4.2.5: Use underscore IDs directly (SVG uses note_X format)
            const firstSvgId = firstMapping.mainNoteId;
            const lastSvgId = lastMapping.noteIds[lastMapping.noteIds.length - 1];
    ...
    // Shared playback functions (to be integrated with lyrics-controller)
    playPhrase(phraseId) {
        console.log(`[PhraseBars] Play phrase ${phraseId}`);
        // Delegate to lyrics controller
        if (window.lyricsController) {
            window.lyricsController.playPhrase(phraseId);
        }

        // Also highlight the phrase bar
        this.highlightPhraseBar(phraseId);
    }

    stopPhrase(phraseId) {
        console.log(`[PhraseBars] Stop phrase ${phraseId}`);
        if (window.lyricsController) {
