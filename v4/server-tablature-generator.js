    ...
    generateSVG(songData, tuningKey, showBentNotes = true) {
        ...
        notes.forEach(note => {
            if (note.isGrace) {
                ...
                // Advance by full main note duration
                currentX += note.duration * this.DURATION_MULTIPLIER;
            }
        });
        ...
        const width = lastMainNote ? lastMainNote.x + lastMainNote.duration * this.DURATION_MULTIPLIER + 400 : 2000;
        const height = 800;
    ...
    generateNotesWithTriangles(strings, notes, showBentNotes) {
        ...
        notes.forEach((note, index) => {
            ...
            if (isGrace && note.graceType) {
                ...
                svg += `    <line x1="${slashX1}" y1="${slashY1}" x2="${slashX2}" y2="${slashY2}" stroke="#FFD700" stroke-width="2" class="grace-slash"/>\n`;
            }
